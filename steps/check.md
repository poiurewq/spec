...
Read state.yaml first; validate phase. Write state.yaml if convergence is declared.

## Protocol

1. Read state.yaml; validate phase.
...
5. **Structural →** not converged. Recommend `/spec review` in a new conversation. Phase stays `revised`. No state.yaml update needed.

6. **Wording-only →** check the revision before that (same two questions).
   - Also wording-only → **convergence declared**. Go to step 7.
   - Structural → one more review/revise cycle, then re-check. Phase stays `revised`.
...
7. **On convergence:**
   ...
   - Update state.yaml: `phase: converged`, `last_command: /spec check`, `last_command_at`.
   - Propose:
     ```
