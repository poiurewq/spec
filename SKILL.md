...
6. **Sub-agent model is Sonnet; always pass `background: false`.** Pass `model: "sonnet"` in every Agent invocation — no step uses an Opus sub-agent; seed (the only Opus-quality drafting step) runs in-conversation in a fresh session, not as a sub-agent. Steps that use `subagent_type: "Explore"` (verify, implement audit, adopt, reconcile scan, interview code summary) do so only after a **triage step**. Triage has two independent direct-path gates — **either is enough to skip Explore**:
